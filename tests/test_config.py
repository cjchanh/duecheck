...
def test_init_creates_private_config(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
...
def test_init_does_not_echo_token(
    tmp_path: Path,
...
def test_init_fails_without_tty(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
):
...
def test_init_warns_when_posix_permissions_unavailable(
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
):
    run_init(
