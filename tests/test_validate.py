"""Tests for DueCheck artifact validation and safe writes."""
...
def test_artifact_missing_schema_version(tmp_path: Path):
    run_demo(tmp_path)
...
def test_artifact_missing_engine_version(tmp_path: Path):
    run_demo(tmp_path)
    ledger = _load_json(tmp_path / "ledger.json")
    delta = _load_json(tmp_path / "delta.json")
    risk = _load_json(tmp_path / "risk.json")
...
def test_validate_rejects_corrupt_ledger(tmp_path: Path):
    run_demo(tmp_path)
    ledger = _load_json(tmp_path / "ledger.json")
    ledger[0]["name"] = ""
...
def test_validate_rejects_wrong_schema_version(tmp_path: Path):
    run_demo(tmp_path)
    delta = _load_json(tmp_path / "delta.json")
...
def test_verify_command_catches_invalid(tmp_path: Path):
    run_demo(tmp_path)
    risk = _load_json(tmp_path / "risk.json")
...
def test_invalid_artifacts_do_not_overwrite_last_good_artifacts(tmp_path: Path):
    run_demo(tmp_path)
    before = {
...
def test_severity_label_rename_no_orphan_confidence(tmp_path: Path):
    run_demo(tmp_path)
